...
const char *			CL_FILE_NAME = { "proj7-opencl.cl" };
...
main( int argc, char *argv[ ] )
{
    ...
    int i;
    int Size;
    fp_signal = fopen( "signal.txt", "r" );
    if( fp_signal == NULL )
    {
        fprintf( stderr, "Cannot open file 'signal.txt'\n" );
        exit( 1 );
    }
    fscanf( fp_signal, "%d", &Size );
    Size = SIZE;
    for( i = 0; i < Size; i++ )
    {
            fscanf( fp_signal, "%f", &hArray[i] );
	...
	status = clEnqueueWriteBuffer( cmdQueue, dArray, CL_FALSE, 0, Size, hArray, 0, NULL, NULL );
	if( status != CL_SUCCESS )
	...
	cl_kernel kernel = clCreateKernel( program, "ArrayReduction", &status );
	if( status != CL_SUCCESS )
