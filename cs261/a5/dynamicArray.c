...
int _smallerIndexHeap(DynArr *heap, int i, int j)
{
	if(i < sizeDynArr(heap) && j < sizeDynArr(heap)){								// FIXME 
...
{
	int leftIdx = ((pos*2) + 1);
	int rightIdx = ((pos*2) + 2);											/* FIXME */

	if(rightIdx < max){
		if(heap->data[leftIdx] != NULL && heap->data[rightIdx] != NULL){
			int minIndex;
			 minIndex = _smallerIndexHeap(heap, leftIdx, rightIdx);
...
void _buildHeap(DynArr *heap)
{
	...
	for(int i = ((sizeDynArr(heap)/2)-1); i >= 0; i--){				/* FIXME */
...
void sortHeap(DynArr *heap)
{
	int max = sizeDynArr(heap);
	_buildHeap(heap);	
	for(int i = sizeDynArr(heap)-1; i >= 0; i--){						// FIXME 
	swapDynArr(heap, 0, i); 
